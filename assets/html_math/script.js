    ...
    getScript(url, function () {
        if (typeof (language) != "undefined") {
            ...
            $('img[src*=en]').each(function (key, val) {
                if(lang !== 'en') {
                    var imgPathOfLocale = "file:///mnt/sdcard/MathsFormulas/unzipFolder/" + lang + '/' + $(val).attr('src');
                    $(val).attr('src', imgPathOfLocale.replace('en', lang));
                }
            });
            capitalizeEachWord('h2', func);
    ...
    handler: {
        keyup: function (obj, e) {
            ...
            if (obj.keyCode !== 189) {
                ...
                $(this).val(value);
                if (!isNaN(value.replace(',', '.')) && value != "" && !conversion.bypass(value)) {
                    value = conversion.reformat(value);
